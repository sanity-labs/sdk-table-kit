...
import {Box, Button, Card, Flex, Label, Popover, Stack, Text, useClickOutsideEvent} from '@sanity/ui'
...
export function TaskSummaryEditorView({
  ...
  const {createTask, editTask, removeTask, toggleTaskStatus} = useAddonTaskMutations()
  ...
  const [dueDateValue, setDueDateValue] = useState(() => (task ? getDateInputValue(task.dueBy) : ''))
  ...
  const isOverdue = isDateValueOverdue(dueDateValue || undefined, taskStatusForOverdue)
  ...
  const ensureTaskId = useCallback(async (): Promise<string> => {
    ...
    const promise = (async () => {
      ...
      latestDraftRef.current = {description: draftDescription, title: draftTitle}
  ...
  const saveDescriptionNow = useCallback(
    async (value: string) => {
      ...
      const savePromise = editTask(taskId, {description: buildMessageFromPlainText(value)})
        .then(() => {
  ...
  }, [effectiveTaskId, onBack, onDeleteOptimistic, onDeleteRollback, onInternalInteraction, removeTask])
