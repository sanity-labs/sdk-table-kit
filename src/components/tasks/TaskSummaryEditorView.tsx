...
import {Bell, BellOff, Calendar, CheckCircle2, ChevronLeft, CircleDashed} from 'lucide-react'
...
export function TaskSummaryEditorView({
  ...
  const [optimisticTaskStatus, setOptimisticTaskStatus] = useState<TaskStatus | null>(null)
  ...
  const effectiveTaskStatus: TaskStatus = optimisticTaskStatus ?? task?.status ?? 'open'
  const isOverdue = isDateValueOverdue(dueDateValue || undefined, effectiveTaskStatus)
  ...
  useEffect(() => {
    if (optimisticTaskStatus !== null && task?.status === optimisticTaskStatus) {
      setOptimisticTaskStatus(null)
    }
  ...
  const ensureTaskId = useCallback(async (): Promise<string> => {
    ...
    const promise = (async () => {
      ...
      const {description: draftDescription, title: draftTitle} = latestDraftRef.current
  ...
  const handleSelectDueDate = useCallback(
    async (date: Date | undefined) => {
      ...
      editTask(taskId, {dueBy: toDueDateIsoString(nextDueDateValue)}).catch((error) => {
  ...
  }, [currentResourceUserId, editTask, ensureTaskId, isSubscribed, task?.subscribers, toast])
  ...
  const handleSelectTaskStatus = useCallback(
    ...
    [editTask, ensureTaskId, onInternalInteraction, optimisticTaskStatus, task?.status, toast],
  )
    ...
    <Stack space={4}>
      ...
      <Flex align="center" gap={2} style={{flexWrap: 'wrap'}}>
        ...
        >
          <Button
            fontSize={1}
            icon={assignee ? <TaskUserAvatar user={assignee} /> : <CircleDashed size={12} />}
            mode="ghost"
                    ...
                    onClick={() => {
                      void (async () => {
                        ...
                        editTask(taskId, {dueBy: ''}).catch((error) => {
                          console.error('[TaskSummaryEditorView] clearDueDate failed', {
                            error,
                            taskId,
                          })
                          setDueDateValue(previousDueDateValue)
  ...
  return (
    <Stack space={3}>
      <Flex align="center" justify="space-between">
        ...
        <Button disabled icon={<BellOff size={16} />} mode="bleed" padding={2} text="Subscribe" />
      </Flex>
      <Card border padding={3} radius={2} tone="transparent">
        <Text muted size={1}>
          Add details to start this task — comments will appear here once the task is created.
        </Text>
      </Card>
    </Stack>
  )
